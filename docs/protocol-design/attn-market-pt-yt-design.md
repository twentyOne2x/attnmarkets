...
- **Creator Opt-In or CTO**: Creators opt in directly or a user (community, business, or DAO) files a Pump.fun CTO request (Google/X form) documenting active stewardship. Upon approval, Pump executes `set_creator` / `set_creator_authority`, pointing the bonding-curve creator PDA to the CreatorVault Squads Safe (user+attn, 2-of-2) nominated in the application.
...
- **Time.fun & Centralized Launchpads (LetsBonk, etc.)**: Fees accrue within platform custody and are forwarded based on off-chain routing. These systems do not expose immutable on-chain fee recipients, so enforcement demands bespoke partnerships or migrating the fee stream into CreatorVault safe (Squads 2-of-2 user+attn).
