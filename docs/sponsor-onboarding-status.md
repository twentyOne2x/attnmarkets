...
## Overview
- Updated the sponsor onboarding flow to honour the new Playwright-based test harness.
...
- Added a 10 s timeout screen for the sponsor page so users aren’t stuck on the loading spinner indefinitely.
...
- **Tour state**
  ...
  - Legacy single-key storage (`attn.liveSponsorTour`) is migrated on both the sponsor page and onboarding component mount.
- **Safe detection**
...
- **Wallet bootstrapping**
  - AppContext still flips back to “no wallet connected” even when tests seed `currentUserWallet`. The sponsor checklist remains at step 1.
...
1. Adjust AppContext initialisation to respect a pre-seeded wallet (from localStorage or an env flag) so the sponsor page can progress without manual clicks.
